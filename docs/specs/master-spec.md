...
- **CROC-003a** — CI checks pipeline: GitHub Actions running `gofmt`,
  ...
  bookkeeping at all), and `go test ./...` — mirrors
  ...
  wired up now even though no repository-layer test needs it yet (`CROC-002`
  deferred all repository code; `CROC-004` is the first ticket that will) —
  avoids a second CI-touching PR later. Added retroactively (after
  ...
  deliberate correction, not a default.

  **Trigger scope deliberately narrower than the reference.**
  `packing-list-go`'s workflow runs on `push`/`pull_request` with no
  branch filter — every push to every branch, plus every PR (so a branch
  with an open PR double-triggers). This project scopes it to
  `pull_request` (targeting `main`) plus `push` on `main` only: every PR
  still gets gated, a direct push to `main` (during the transition before
  branch protection is fully adopted as a habit) still gets checked, but
  routine feature-branch pushes don't burn Actions minutes running a
  check that gates nothing until a PR exists.

  **Branch protection on `main`, configured by hand in GitHub (not code):**
  "Require a pull request before merging" (0 required approvals — solo
  repo, GitHub won't let you approve your own PR anyway), "Require status
  checks to pass before merging" with the `checks` job selected once it's
  run at least once, and "Do not allow bypassing the above settings" so
  the repo's sole admin isn't silently exempt from their own rule.
  Decided deliberately, not defaulted to: required status checks alone
  don't block a direct push to a protected branch in any meaningful way —
  the PR requirement is what actually forces the gate; the status check
  alone would just leave a red X nobody's forced to look at.

  *AC: a PR with a deliberately unformatted file, a `go vet` issue, a
  known-vulnerable dependency, a stale `go.mod` (a direct import wrongly
  left `// indirect`), and a failing test each independently fail CI and
  leave the PR unmergeable; a clean PR passes all five checks and the
  merge button unlocks. A direct push attempt to `main` is rejected.*

  *Verification: exercised through the real pipeline, not read off the
  YAML — open real test PRs against real GitHub Actions, one per broken
  condition above, confirming each fails the right check; then a clean
  PR confirming all five pass. Branch-protection enforcement is checked
  by hand (attempt an actual merge with a failing check and confirm the
  button stays disabled; attempt an actual direct push to `main` and
  confirm it's rejected) — this is exactly the class of thing tests and
  screenshots can't verify, only using the real GitHub UI can.*
